    ...
    def __or__(self, other: "NooiceSlice") -> "NooiceSlice":
        ...
        if self.disjoint(other):
            raise NotImplementedError(
                "A union of disjoint NooiceSlices is not a NooiceSlice."
            )
...
class CubeSlice:
    def __init__(
        self, on: bool, xslice: NooiceSlice, yslice: NooiceSlice, zslice: NooiceSlice
    ):
        self.on = on
        self.x = xslice
        self.y = yslice
    ...
    def adjacent(self, other: "CubeSlice") -> bool:
        return (
            self._adjacent_x(other)
            or self._adjacent_y(other)
            or self._adjacent_z(other)
        )
...
def parse_input(s: str) -> list[CubeSlice]:
    cube_slices = re.findall(
        r"(on|off) x=(-*\d+)..(-*\d+),y=(-*\d+)..(-*\d+),z=(-*\d+)..(-*\d+)", s
    )
    slices = []
...
class Cube:
    ...
    def add_slice(self, new_slice: CubeSlice):
        intersecting_partitioned_slices = [
            pslice for pslice in self.__slices if not pslice.disjoint(new_slice)
        ]
        ...
        if new_slice.off:
            ...
            self.__slices.extend(
                self.partition_slices([new_slice] + intersecting_partitioned_slices)
            )
                ...
                if not pslice.disjoint(new_slice):
                    ...
                    new_slices.extend(
                        merge_adjacent_cubes(
                            list(generate_all_products(pslice, new_slice))
                        )
                    )
                    break
...
def generate_all_products(cube1: CubeSlice, cube2: CubeSlice) -> Generator:
    ...
    for (xs_new, xe_new), (ys_new, ye_new), (zs_new, ze_new) in product(
        xs_pairs, ys_pairs, zs_pairs
    ):
        ...
        if (xs_new, ys_new, zs_new) in cube1 and not (
            cube2.off and (xs_new, ys_new, zs_new) in cube2
        ):
            yield CubeSlice(
                True,
