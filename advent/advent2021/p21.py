...
def prune_winners(
    state_dict: Counter[WorldState, int], winning_count: tuple[int, int]
) -> Counter[WorldState, int]:
...
def transition_state(
    state: WorldState, state_dict: Counter[WorldState, int], pnum: int
):
...
def solve_a(s: str) -> int:
    (_, p1_start), (_, p2_start) = re.findall(
        r"Player (\d+) starting position: (\d+)", s
    )
