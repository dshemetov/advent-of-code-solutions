    ...
    def process_op(self, op, inp, register):
        ...
        if op == "inp":
            ...
            for key2 in self.back_vars:
                self.back_vars[key2] = self.back_vars[key2].replace(
                    register, input_variable
                )
        ...
        if op == "mul":
            if val == 0:
                ...
                for key in self.back_vars:
                    self.back_vars[key] = self.back_vars[key].replace(
...
def parse_input(s: str):
    program_instructions = findall(
        r"(inp) (\w)|(add|mod|mul|eql|div) (\w) (-*\d+|\w)", s
    )
    program_instructions = [
        [x if x.isalpha() else int(x) for x in instruction if x != ""]
