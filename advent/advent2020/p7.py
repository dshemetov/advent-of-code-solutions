...
def parse_line(line: str) -> tuple[str, str]:
    """
    Examples:
    >>> parse_line("light salmon bags contain 5 wavy plum bags, 4 drab white bags, 5 muted bronze bags, 5 mirrored beige bags.")
    ('light salmon', [[5, 'wavy plum'], [4, 'drab white'], [5, 'muted bronze'], [5, 'mirrored beige']])
    """
    container_bag, contained_bags = re.match(
        r"(\w+ \w+) bags contain (.*)", line
    ).groups()
    contained_bags = [
        [int(e.groups()[0]), e.groups()[1]]
        for e in re.finditer(r"(\d+) (\w+ \w+) bags?", contained_bags)
    ]
