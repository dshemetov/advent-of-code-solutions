class GameState:
    ...
    def advance_game(self, n: int = 1):
        ...
        for _ in range(n):
            new_value = GameState.get_next_number(
                self.current_value, self.turn_number, self.last_turn
            )
    ...
    @staticmethod
    def get_next_number(
        current_value: int, turn_number: int, last_turn: dict[int, int]
    ):
...
def test_get_next_number():
    gs = GameState([0, 3, 6])
    assert (
        GameState.get_next_number(gs.current_value, gs.turn_number, gs.last_turn) == 0
    )
    gs = GameState([0, 3, 6, 0])
    assert (
        GameState.get_next_number(gs.current_value, gs.turn_number, gs.last_turn) == 3
    )
    ...
    assert (
        GameState.get_next_number(gs.current_value, gs.turn_number, gs.last_turn) == 1
    )
