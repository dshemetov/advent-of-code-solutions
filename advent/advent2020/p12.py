...
class State:
    ...
    def __eq__(self, other):
        if isinstance(other, State):
            return np.allclose(self.ship_pos, other.ship_pos) and np.allclose(
                self.waypoint_pos, other.waypoint_pos
            )
        else:
            return False
...
def execute_command_a(
    state: tuple[np.ndarray, np.ndarray], command: tuple[str, str]
) -> np.ndarray:
    pos, directory = state
