...
def solve_a(s: str) -> int:
    ...
    depart_time, bus_schedule = int(depart_time), filter(
        lambda x: x != "x", bus_schedule.split(",")
    )
    bus_id, next_bus_time = min(
        [
            (int(bus_id), get_next_bus_time(depart_time, int(bus_id)))
...
def solve_modular_congruence_equations(residue_classes: list[tuple[int, int]]) -> int:
    ...
    if not all(
        get_gcd(a, b) == 1
        for a, b in product((m for _, m in residue_classes), repeat=2)
        if a != b
    ):
        raise ValueError(
            "The moduli in the system are not all pair-wise relatively prime."
        )
...
def solve_modular_congruence_equation_pair(
