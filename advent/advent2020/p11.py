...
def update_state_until_fixed(state: list[list[str]], method: str) -> list[list[str]]:
    ...
    while no_changes is False:
        no_changes = True
        newly_occupied_seats, newly_vacant_seats = find_seats_to_update(
...
def find_seats_to_update(state: list[list[str]], method: str) -> list[list[str]]:
    if method == "nearby":
        newly_occupied_seats = (
            (i, j)
            for i, j in get_empty_seats(state)
            if check_nearby_seats_empty(i, j, state)
        )
        newly_vacant_seats = (
            (i, j)
            for i, j in get_occupied_seats(state)
            if check_nearby_seats_crowded(i, j, state)
        )
    if method == "visible":
        newly_occupied_seats = (
            (i, j)
            for i, j in get_empty_seats(state)
            if check_visible_seats_empty(i, j, state)
        )
        newly_vacant_seats = (
            (i, j)
            for i, j in get_occupied_seats(state)
            if check_visible_seats_crowded(i, j, state)
        )
...
def get_first_visible_object(
    i: int, j: int, di: int, dj: int, state: list[list[str]]
) -> str:
    n, m = len(state), len(state[0])
    i_, j_ = i + di, j + dj
