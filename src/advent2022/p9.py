...
def print_tails_on_grid(tails, min_x: int = None, max_x: int = None, min_y: int = None, max_y: int = None):
    ...
    m = np.array([[str(tails_to_index.get((x, y), ".")) for x in range(min_x - 1, max_x + 1)] for y in range(max_y + 1, min_y - 1, -1)])
    print(m)
