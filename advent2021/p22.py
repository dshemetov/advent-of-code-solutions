...
class NooiceSlice:
    ...
    @classmethod
    def check_start_stop(cls, start: Optional[int], stop: Optional[int]) -> None:
        if stop < start:
...
class CubeSlice:
    ...
    def points_iter(self) -> Generator:
        for x, y, z in product(range(self.x.start, self.x.stop), range(self.y.start, self.y.stop), range(self.z.start, self.z.stop)):
            yield (x, y, z)
